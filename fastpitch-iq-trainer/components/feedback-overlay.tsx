"use client"

import type { FeedbackData } from "@/lib/types"
import { cn } from "@/lib/utils"
import { Check, X } from "lucide-react"

interface FeedbackOverlayProps {
  feedback: FeedbackData
  onContinue: () => void
}

export function FeedbackOverlay({ feedback, onContinue }: FeedbackOverlayProps) {
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-foreground/50 backdrop-blur-sm animate-in fade-in duration-200"
      onClick={onContinue}
    >
      <div
        className="bg-card rounded-2xl shadow-xl p-6 max-w-sm w-full animate-in zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Status badge */}
        <div className="flex justify-center mb-4">
          <div
            className={cn(
              "w-16 h-16 rounded-full flex items-center justify-center",
              feedback.correct ? "bg-success/20" : "bg-destructive/20",
            )}
          >
            {feedback.correct ? <Check className="w-8 h-8 text-success" /> : <X className="w-8 h-8 text-destructive" />}
          </div>
        </div>

        {/* Status text */}
        <h2
          className={cn("text-xl font-bold text-center mb-2", feedback.correct ? "text-success" : "text-destructive")}
        >
          {feedback.correct ? "Correct!" : "Not quite"}
        </h2>

        {/* Correct answer */}
        <p className="text-center text-card-foreground mb-2">
          <span className="text-muted-foreground">Answer: </span>
          <span className="font-semibold">{feedback.correctAnswer}</span>
          {feedback.correctTarget && <span className="text-muted-foreground"> → {feedback.correctTarget}</span>}
        </p>

        {/* Explanation */}
        <p className="text-sm text-muted-foreground text-center mb-6">{feedback.explanation}</p>

        {/* Continue button */}
        <button
          onClick={onContinue}
          className="w-full py-3 rounded-xl bg-primary text-primary-foreground font-semibold transition-all active:scale-[0.98]"
        >
          Continue
        </button>
      </div>
    </div>
  )
}
